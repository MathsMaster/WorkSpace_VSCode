/**
 * @brief 利用linux内核模块的方法 , 实现一个/proc文件系统
 *      需求: 在内存中，实现一个伪文件系统proc,它只存在于内存中，不占用外存空间
 *          利用内核模块，在/proc下创建proc_example目录, 并且在该目录下创建三个普通文件夹（foo,bar,jiffies）,和一个文件链接jiffies_too
 *      思路: ???
 *
 * @version 0.1
 * @date 2022-05-14
...
//这些引用的函数库，都在linux系统的 /usr/include/linux下
...
//用于描述文件的结构体 , 这里的文件到底是几级目录构建的 , 还是说直接就是多级树形目录
struct fb_data_t
{
...
static int __init init_procfs_example(void)
{
    ...
    //=========创建目录===========
    example_dir = proc_mkdir(MODULE_NAME, NULL); //这个操作函数表示啥？？？？？
    if (example_dir == NULL)
    {
        rv = -ENOMEM;
        goto out;
...
//==============模块cleanup函数==============
static void __exit cleanup_procfs_example(void) //用的都是Linux内核的一套接口操作函数,和C语言里面的函数是两码事啊
{
    remove_proc_entry("jiffies_too", example_dir);
    remove_proc_entry("bar", example_dir);
    remove_proc_entry("foo", example_dir);
...
MODULE_LICENSE("GPL");
module_init(init_procfs_example);    //装入内核模块命令时执行这
module_exit(cleanup_procfs_example); //卸载内核模块命令时执行这
